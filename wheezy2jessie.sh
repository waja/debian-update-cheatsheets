...
apt-get autoremove && \
apt purge $(aptitude search ?obsolete | grep -v -E 'linux-image|check-openmanage|mailscanner|hp-health|hpacucli' | awk '/^i *A/ { print $3 }') && \
apt purge $(aptitude search ?obsolete | grep -v -E 'linux-image|check-openmanage|mailscanner|hp-health|hpacucli' | awk '/^i/ { print $2 }') && \
