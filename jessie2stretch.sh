...
cat >  /etc/apt/sources.list.d/archive.debian.org.list<<EOF
#
# jessie
#
...
source /etc/default/config-mysql-server-auth; mysql_upgrade -f --password=${MYSQLPW}
...
apt purge $(aptitude search ?obsolete | grep -v -E 'linux-image|mailscanner|kerio|hpacucli|check-openmanage|check-linux-bonding|hp-health' | awk '/^i *A/ { print $3 }') && \
apt purge $(aptitude search ?obsolete | grep -v -E 'linux-image|mailscanner|kerio|hpacucli|check-openmanage|check-linux-bonding|hp-health' | awk '/^i/ { print $2 }') && \
